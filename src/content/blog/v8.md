...
There have been other compilers outside of V8 that are used for Node.js and some are even still developed, like SpiderMonkey. [[2]](https://spidermonkey.dev/) But V8 is by far the most prevalant. As a disclaimer, it's actually a bit unfair to refer to V8 as a compiler, because that is only one half of what V8 does.
...
1. Parsing
   - You can think of this as our _"Front End"_
   - **Input**: Source Code
   - **Output**: Abstract Syntax Tree
2. Ignition
   - You can think of this as our _"Middle End"_
   - **Input**: Abstract Syntax Tree
   - **Output**: Bytecode
3. Turbofan
   - You can think of this as our _"Back End"_
   - **Input**: Bytecode
   - **Output**: Machine Code
...
Parsing is a core process of any compilation process. Typically during the parsing process, we are going to perform additional checks, like ensuring that the program source code follows syntax rules or semantic rules. We will be skipping past those pieces and focusing just on the parsing piece itself. Parsing can generally be viewed as the construction of an Abstract Syntax Tree (AST) from source code. The AST is an exact representation of the underlying source code. Which is to say that at this point, we have not performed any optimizations and it's a 1:1 representation. Let's look at a rather simplistic example.
...
Now we haven't fully reached intermediate languages yet, but the same concept applies. This is a series of statements in which we have a clear idea of the beginning and the end. Flow of control in this context refers to the same constructs you are used to from all programming languages: branches, loop and gotos. The final piece, is our introduction of scopes. These scopes are used in a compiler analysis phase called **liveness analysis**, among others. As I stated previously, any reasonable compiler will figure out that almost none of this function is relevant and scopes are a way that the compiler will resolve that relevancy.
...
// Probably Invoked Function Expression
!function () {
  console.log("hello world");
};
...
I want to call out particular attention to the first example. This is an example of an "Immediately Invoked Function Expression", also called an IIFE. [[4]](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) IIFEs are a common tool in the Javascript optimization arsenal and for those that remember, it used to be primary way to handle top level awaits. Whereas these are going to be examples of **Lazy** parsing.
...
Lazy parsing still has it's place in the Javascript ecosystem, and there can be immense benefits from waiting until a function is actually invoked to perform the complete parse. Estimates still put lazy parsing at around 2x faster than it's eager counterpart. However, in practice, that has proved to be difficult to achieve and required a very strong control of the execution tree for your system. This strong control proved difficult for many systems in the past. Lazy parsing used to be a bit of an issue, so much so that a library called `optimize-js` would convert functions into `PIFE`s. Which is an acronym for _possibly invoked function expressions_, based on static heuristics that would result in non-neglible performance improvements. [[6]](https://www.mattzeunert.com/2017/01/30/lazy-javascript-parsing-in-v8.html). However, starting in 2019, the v8 team added optimizations that negated most of the performance benefits of `optimize-js` and the project became deprecated. [[7]](https://v8.dev/blog/preparser#pife).

The idea of lazy parsing still remains in web performance. That is to say, there has been a common acronym present in performance for web development for some time: **PRPL**. [[5]](https://web.dev/apply-instant-loading-with-prpl/) That acronym stands for:
...
The idea being that the **L** in **PRPL** would be where the value in lazy parsing comes into play from the Javascript side. If you don't need it immediately, you should be able to defer it to a later time period where it becomes useful. As an example, the website you are viewing this on defers features like the search capability, the automatic highlight of sections and tag selection until after the client has completely loaded the site.

# Ignition

Now that we have a decent understanding of the critical part of the front-end, we want to characterize the next phase of the compiler, the middle-end. The middle-end generally is reasonable for creating an intermediate representation. Ideally, this intermediate representation is an interface that can be used by any backend that understands it. The most common representation of an intermediate representation output is something called "bytecode".

> Bytecode, also termed portable code or p-code, is a form of instruction set designed for efficient execution by a software interpreter [[9]](https://en.wikipedia.org/wiki/Bytecode)

And Ignition is something that interprets bytecode.

![TODO](../../assets/v8/pipeline.png "Placeholder")

> Ignition is a fast low-level register-based bytecode interpreter written using the backend of TurboFan [[8]](https://v8.dev/docs/ignition)

So the question that remains is how do we generate bytecode?

## Bytecode

> The BytecodeGenerator walks this AST and generates bytecode for each of the AST nodes as appropriate. [[10]](https://docs.google.com/document/d/11T2CRex9hXxoJwbYqVQ32yIPMh0uouUZLdyrtmMoL44/edit?ts=56f27d9d#heading=h.6jz9dj3bnr8t)

Now unfortunately, we will be deviating from our normal dive into the specifics of the code. The **ByteCodeGenerator** is an over 6,000 line file that for all intents and purposes isn't that exciting. The process of generating bytecode is relatively straightforward. It's something that any undergraduate compilers course will teach you in the first several weeks of the course. Bytecode generation has never been the most complex portion of the compilation process. So let's look at a post-processed example of bytecode.
...
This is one our examples from the simple parsing before. It's a straightforward example with enough of the complexity to discuss at a deeper level. One of the most difficult things you will run into at an introductory level in compilers is function call invocation. So let's explore the bytecode.
...
Let's visualize this slightly to add more context around what is happening. In the following image, we are doing the following.

1. Load "console" into the accumulator
2. Store "console" in the r1 register
3. Load "console.log" into the accumulator
4. Store "console.log" in the r0 register
5. Load the constant "hello world" into the accumulator
6. Store the value in the accumulator in r2
7. Call the function at r0 with context of "console" as dictated by r1, with the argument "hello world" as dictated by r2
8. Load undefined into the accumulator
...
5. https://web.dev/apply-instant-loading-with-prpl/
6. https://www.mattzeunert.com/2017/01/30/lazy-javascript-parsing-in-v8.html
7. https://v8.dev/blog/preparser#pife
8. https://v8.dev/docs/ignition
9. https://en.wikipedia.org/wiki/Bytecode
10. https://docs.google.com/document/d/11T2CRex9hXxoJwbYqVQ32yIPMh0uouUZLdyrtmMoL44/edit?ts=56f27d9d#heading=h.6jz9dj3bnr8t

11. https://www.youtube.com/watch?v=PsDqH_RKvyc
12. https://doar-e.github.io/blog/2019/01/28/introduction-to-turbofan/#compilation-pipeline
13. https://docs.google.com/presentation/d/1H1lLsbclvzyOF3IUR05ZUaZcqDxo7_-8f4yJoxdMooU/edit#slide=id.g18ceb14729_0_48
14. https://blog.logrocket.com/how-javascript-works-optimizing-the-v8-compiler-for-efficiency/
15. https://ponyfoo.com/articles/an-introduction-to-speculative-optimization-in-v8

16. https://www.youtube.com/watch?v=Fg7niTmNNLg
17. https://v8.dev/blog/scanner
18. https://v8.dev/blog/lazy-unlinking
19. https://www.youtube.com/watch?v=u7zRSm8jzvA
20. https://aerotwist.com/blog/when-everything-is-important-nothing-is/
21. https://blog.chromium.org/2015/03/new-javascript-techniques-for-rapid.html

22. https://docs.google.com/presentation/d/1Lq2DD28CGa7bxawVH_2OcmyiTiBn74dvC6vn2essroY/edit#slide=id.g1a504e63c9_2_89
23. https://v8.dev/blog/cost-of-javascript-2019
24. https://www.youtube.com/watch?v=cvybnv79Sek
25. https://mrale.ph/blog/2015/01/11/whats-up-with-monomorphism.html
26. https://mrale.ph/blog/2012/06/03/explaining-js-vms-in-js-inline-caches.html
27. https://mrale.ph/s3/webrebels2012.pdf
28.
29. https://medium.com/dailyjs/understanding-v8s-bytecode-317d46c94775
30. https://www.youtube.com/watch?v=p-iiEDtpy6I
31. https://docs.google.com/presentation/d/1OqjVqRhtwlKeKfvMdX6HaCIu9wpZsrzqpIVIwQSuiXQ/edit#slide=id.g1357e6d1a4_0_58
32. https://blog.sessionstack.com/how-javascript-works-parsing-abstract-syntax-trees-asts-5-tips-on-how-to-minimize-parse-time-abfcf7e8a0c8
33. https://www.jfokus.se/jfokus18/preso/Escape-Analysis-in-V8.pdf
34. https://v8.dev/docs/turbofan
35. https://channel9.msdn.com/Shows/Going+Deep/Expert-to-Expert-Erik-Meijer-and-Lars-Bak-Inside-V8-A-Javascript-Virtual-Machine
